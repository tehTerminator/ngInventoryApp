...
export class InvoiceStoreService {
  ...
  addTransaction(transaction: Transaction) {
    ...
    const indexOfSimilarTransaction = this.findSimilarTransaction(
      transactionData,
      transaction
    );
    if (indexOfSimilarTransaction > 0) {
      transactionData[indexOfSimilarTransaction].quantity +=
  ...
  private findSimilarTransaction(
    ...
    return data.findIndex(
      (x) =>
        x.product_id === transaction.product_id && x.rate === transaction.rate
    );
  }
